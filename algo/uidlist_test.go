...
func BenchmarkListCreation(b *testing.B) {
	randomTests := func(sz int) {
		b.Run(fmt.Sprintf(":WriteIterator:size=%d:", sz),
			func(b *testing.B) {
				for k := 0; k < b.N; k++ {
					ub := new(task.List)
					it := NewWriteIterator(ub, 0)
					for i := 0; i < sz; i++ {
						it.Append(100)
					}
					it.End()
		...
		b.Run(fmt.Sprintf(":PreAllSlice:size=%d:", sz),
			func(b *testing.B) {
				for k := 0; k < b.N; k++ {
					list := make([]uint64, sz)
					for i := 0; i < sz; i++ {
		...
		b.Run(fmt.Sprintf(":AppendSlice:size=%d:", sz),
			func(b *testing.B) {
				for k := 0; k < b.N; k++ {
					var list []uint64
					for i := 0; i < sz; i++ {
		...
		for _, r := range rs {
			...
			*/
			...
			if len(f1) != len(u) {
				...
				b.Logf("f1: %v\n", f1)
				//b.Logf("f2: %v\n", f2)
				b.Fatalf("lengths are different: %v %v %v %v %v", len(u), len(f1))
			}
			for i := range u {
				if f1[i] != u[i] {
					b.Fatalf("Uids are different at idx: %v: %v %v %v", i, u[i], f1[i])
				}
			}
			fmt.Println()
			/*
